...
type InternalEventCallbackType =
  ...
  | 'onClose'
  | 'onRequestRetry'
  | 'onResultCacheEviction';

// Optional callbacks for internal events useful for debugging and testing
export type InternalEventCallbacks = { [key in InternalEventCallbackType]: (...args: any[]) => void };
...
export class MiniIframeRPC {
  ...
  public constructor(initParameters?: Partial<InitParameters>) {
    this.config = {
      windowRef: (initParameters && initParameters.windowRef) || window,
      originWhitelist: (initParameters && initParameters.originWhitelist) || [],
      defaultInvocationOptions: Object.assign(
        {},
        DEFAULT_INVOCATION_OPTIONS,
        (initParameters && initParameters.defaultInvocationOptions) || {}
      ),
      eventCallbacks: (initParameters && initParameters.eventCallbacks) || {},
      resultCacheCapacity:
        initParameters && typeof initParameters.resultCacheCapacity === 'number'
    ...
    this.transport = new PostMessageTransport(this.config.windowRef, this.recv, {
  ...
  private timeboxPromise(originalPromise: Promise<any>, timeoutMilliSeconds: number): Promise<any> {
    return Promise.race([
      ...
      new Promise((_resolve, reject) => {
        this.config.windowRef.setTimeout(() => reject(new TimeoutError({ timeoutMilliSeconds })), timeoutMilliSeconds);
      })
    ]);
  }
  ...
  private async handleRequest(requestMessageBody: RequestMessageBody, messageSource: Window, messageOrigin: string) {
    ...
    if (this.registeredProcedures[method]) {
      getResult().then(
        (result?: any) =>
          this.sendMessage(messageSource, responseOrigin, {
            id,
            result
